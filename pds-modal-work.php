...
if(isset($_POST['save_work'])){
    ...
    foreach ($workDetails as $work) {
        $userID = $_SESSION['id'];
        $workfrom = $work['up_workfrom'];
        $workto = $work['up_workto'];
        $department = $work['up_department'];
        $salary = $work['up_salary'];
        $paygrade = $work['up_paygrade'];
        $appointment = $work['up_appointment'];       
        $govservice = $work['up_govservice'];       

        $queryUpdateworkDetails = "INSERT INTO `work_experience_tb` (userid, work_from_date, work_to_date, department, salary, paygrade, appointment, gov_service) VALUES (?,?,?,?,?,?,?,?,?)";
        $stmt_updateWorkDetails = $con->prepare($queryUpdateworkDetails);

        $stmt_updateWorkDetails->bind_param('isssssss','$userID', '$workfrom', '$workto', '$department', '$salary', '$paygrade', '$appointment', '$govservice');
...
$update_Work_Details = isset($_SESSION['updateWorkDetails']) ? $_SESSION['updateWorkDetails'] : array();
                    ...
                    <div class="work-details-div m-0 overflow-scroll">                          
                        <table class="table table-secondary table-striped text-center mt-3  work">
                            ...
                            <tbody>
                                <?php
                                    $userID = $_SESSION['id'];
                                    $querySelectWork = "SELECT * FROM `work_experience_tb` where userid = $userID";
                                    $sqlSelectWork = mysqli_query($con,$querySelectWork);

                                    while($rowUpdateWork = mysqli_fetch_array($sqlSelectWork)){
                                        echo '<tr>
                                        <td>'.$rowUpdateWork['work_from_date'].'</td>
                                        <td>'.$rowUpdateWork['work_to_date'].'</td>
                                        <td>'.$rowUpdateWork['department'].'</td>
                                        <td>'.$rowUpdateWork['salary'].'</td>
                                        <td>'.$rowUpdateWork['paygrade'].'</td>
                                        <td>'.$rowUpdateWork['appointment'].'</td>
                                        <td>'.$rowUpdateWork['gov_service'].'</td>
                                        </tr>';
                                    }
                                ?>                                
                            </tbody>                            
